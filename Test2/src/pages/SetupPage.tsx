...
import { defaultAvailability, studyBlockCount, weeklyStudyMinutes } from '../utils/timeMap';
...
export function SetupPage({ onComplete }: { onComplete: (profile: UserProfile) => void }) {
  ...
  const submit = () => {
    ...
    onComplete({
      ...
      professionalYear,
      curriculumVersion: 'NCISM current / selected professional year',
      sessionMinutes,
      breakMinutes,
      maxDailyMinutes,
      focusPeriod,
          ...
          <section>
            ...
            <label>
              ...
              <select value={professionalYear} onChange={(event) => setProfessionalYear(event.target.value as ProfessionalYear)}>
                <option>I Professional</option>
                <option>II Professional</option>
                <option>III Professional</option>
