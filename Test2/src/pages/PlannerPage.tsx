...
export function PlannerPage({
  ...
  onUpdatePlan: (planId: string, updater: (plan: StudyPlan) => StudyPlan) => void;
}) {
  ...
  return (
    <div className="page-stack">
      <section className="planner-toolbar">
        ...
        <div className="button-row"><button className="button button--primary button--small" type="button" onClick={() => setCreating(true)}>New plan</button></div>
      </section>
      ...
      {!activePlan ? (
        ...
        <>
          ...
          <section className="section-card">
            <div className="section-heading"><div><span className="eyebrow">Plan controls</span><h3>Manage this plan</h3></div></div>
            <div className="button-row button-row--wrap">
              <button className="button button--ghost button--small" type="button" onClick={() => onUpdatePlan(activePlan.id, (plan) => ({ ...plan, paused: !plan.paused }))}>{activePlan.paused ? 'Resume plan' : 'Pause plan'}</button>
              <button className="button button--ghost button--small" type="button" onClick={() => onUpdatePlan(activePlan.id, (plan) => ({ ...plan, archived: !plan.archived }))}>{activePlan.archived ? 'Restore plan' : 'Archive plan'}</button>
              <button className="button button--ghost button--small" type="button" onClick={() => onDuplicatePlan(activePlan.id)}>Duplicate</button>
              <button className="button button--danger button--small" type="button" onClick={() => onDeletePlan(activePlan.id)}>Delete</button>
            </div>
          </section>
              ...
              {upcomingTasks.map((task) => (
                <article key={task.id} className={`planner-task planner-task--${task.status.toLowerCase()}`}>
                  ...
                  <div className="planner-task__actions">
                    <button className="button button--ghost button--small" type="button" onClick={() => onOpenTask(task.id)}>Open</button>
                    ...
                    <label className="mini-lock"><input type="checkbox" checked={Boolean(task.locked)} onChange={(event) => onUpdatePlan(activePlan.id, (plan) => ({ ...plan, tasks: plan.tasks.map((item) => item.id === task.id ? { ...item, locked: event.target.checked } : item) }))} /> Lock</label>
                  </div>
                </article>
              ))}
