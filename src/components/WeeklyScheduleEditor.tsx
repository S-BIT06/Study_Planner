...
const startOptions = timeOptions(false);
const endOptions = timeOptions(true);
        ...
        {DAYS.map((dayName) => {
          ...
          return (
            ...
            >
              ...
              <span>{studyMinutes ? `${Math.round(studyMinutes / 15) * 15}m` : 'off'}</span>
            </button>
          );
        })}
      </div>
              ...
              <div className="field-grid">
                <label>
                  ...
                  <select value={draft.startTime} onChange={(event) => setDraft({ ...draft, startTime: event.target.value })}>
                    {startOptions.map((time) => <option key={time}>{time}</option>)}
                  </select>
                </label>
                <label>
                  End
              ...
              {isStudyCategory(draft.category) && (
                <div className="field-grid">
                  ...
                  <label className="toggle-card">
                    ...
                    <span><strong>Lock this window</strong><small>The scheduler may use it, but profile rescheduling will keep its position preference.</small></span>
                  </label>
                </div>
              )}
