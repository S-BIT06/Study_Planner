...
import { DateExceptionEditor } from '../components/DateExceptionEditor';
      ...
      {editing ? (
        <section className="section-card profile-form">
          ...
          <div className="field-grid">
            <label>Professional year<select value={draft.professionalYear} onChange={(event) => setDraft({ ...draft, professionalYear: event.target.value as ProfessionalYear })}>{PROFESSIONAL_YEARS.map((year) => <option key={year} value={year}>{year}</option>)}</select></label>
          ...
          <DateExceptionEditor
            value={draft.dateExceptions}
            onChange={(dateExceptions) => setDraft((current) => ({ ...current, dateExceptions }))}
            maxDailyMinutes={draft.maxDailyMinutes}
          />
        ...
        <>
          <section className="section-card">
            ...
            {profile.dateExceptions.length > 0 && <div className="info-banner">{profile.dateExceptions.length} date-specific exception(s) override the weekly map on selected dates.</div>}
          </section>
