...
export function HomePage({
  ...
  const nextTask = todayTasks.find((task) => task.status !== 'COMPLETED' && task.status !== 'SKIPPED')
    ?? plan?.tasks.find((task) => task.date > today && task.status !== 'COMPLETED' && task.status !== 'SKIPPED');
        ...
        {nextTask ? (
          <>
            ...
            <div className="button-row">
              <button className="button button--primary" type="button" onClick={() => { onTaskStatus(nextTask.id, 'IN_PROGRESS'); onOpenTask(nextTask.id); }}>Start session</button>
              <button className="button button--ghost" type="button" onClick={() => onOpenTask(nextTask.id)}>Record progress</button>
            </div>
          </>
      ...
      <section className="section-card">
        ...
        {todayTasks.length ? (
          <div className="timeline-list">
            {todayTasks.map((task) => (
              <button className={`timeline-item timeline-item--${task.status.toLowerCase()}`} key={task.id} type="button" onClick={() => onOpenTask(task.id)}>
